...
mn_clean <- mics6_mn_selected %>%
  mutate(
    ...
    # Standardize education_male and wealth_quintile to factors
...
track1_table_gt <- track1_table %>%
  gt() %>%
  tab_header(
    title = "Demographic Distribution by Media Cluster (Track 1)",
    subtitle = "Percentage of respondents by sex and wealth within each media cluster"
  ) %>%
  fmt_percent(columns = where(is.numeric), scale_values = FALSE)

gtsave(track1_table_gt, filename = "tables/track1_media_clusters_table.html")

gtsave(track1_table_gt, filename = "tables/track1_media_clusters_table.png")
...
track2_table %>%
  ...
  save_kable("tables/track2_risk_clusters_table.html") 
  
# Optional: PDF
# save_kable("tables/track2_risk_clusters_table.pdf")
